...
对于上框和下框参数，我不会在这里展示它们，如果你感兴趣，可以参考我的仓库中的 `draw_lower_box` 和 `draw_upper_box` 函数，许多绘图参数参考了 **DanmakuFactory**[^danmaku_factory]，非常感谢作者。在这里我想谈谈如何根据其他超级弹幕的出现和消失来移动超级弹幕消息框。

[^danmaku_factory]: https://github.com/hihkm/DanmakuFactory

我们可以看到，超级弹幕按照时间顺序出现和消失，那么我们如何在特定时间点定义超级弹幕的位置呢？

我的解决方案是：每次超级弹幕在其生命周期中改变位置时计算超级弹幕消息框的位置。具体的算法如下：
...
但是，这些是最好的显示吗？我认为不是。如果我们只考虑位置，视频会太无聊，所以我们应该给超级弹幕的出现添加一些动画。想想超级弹幕就像一个气泡，它会根据其他超级弹幕的出现和消失而上下移动。图片会更适合观众。

现在我们可以思考气泡移动效果的算法。正如我们上面讨论的那样，这种移动可以通过 `move` 效果来实现。

首先，新的超级弹幕会在屏幕底部出现，所以初始 `y` 位置是 `resolution_y - font_size * 2`（我们应该为礼物弹幕保留一些空间，我将在下一节介绍它们）。超级弹幕的出现位置将是初始 `y` + 消息框高度。

然后，新的超级弹幕如何影响其他仍然存在的超级弹幕？在特定时间点，其他超级弹幕会向上移动新的超级弹幕高度。所以移动参数将是 `current_y` 到 `current_y - new_superchat_height`。

如果存在一个特定的超级弹幕消失，那么比它更早的超级弹幕将向下移动超级弹幕高度。所以移动参数将是 `current_y` 到 `current_y + disappear_superchat_height`。然后一切都变得有意义了。

然后我们应该思考，什么时候开始移动？会有很多解决方案，但我认为在超级弹幕改变位置的时间点开始移动是最好的选择。并使移动持续一段时间，比如 0.2 秒。

所以过程将是：
- 改变位置时间 + 0.2s: 使超级弹幕移动效果。
- 改变位置时间 + 0.2s ~ 下一个改变位置时间: 使超级弹幕 pos 效果。
